...
func save_map(map_baker: MapBaker, atlas_image: Image):
	...
	var save_path: String = export_path.path_join("result_" + map_baker.get_name() + ".png")
...
func export_scene(mat: Material, texture_array: bool = false, shadow_mat: Material = null) -> Node3D:
	...
	mat.set_shader_parameter("imposterFrames", Vector2(frames_xy, frames_xy))
	...
	if shadow_mat != null:
		...
		shadow_mat.set_shader_parameter("imposterFrames", Vector2(frames_xy, frames_xy))
