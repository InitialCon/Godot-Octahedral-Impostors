...
uniform sampler2DArray imposterDepthAlphaTexture : hint_albedo;
...
vec4 blenderColors(vec3 uv_1, vec3 uv_2, vec3 uv_3, vec4 grid_weights, sampler2DArray atlasTexture)
{
...
vec3 recalculateUV(vec2 uv_f, vec2 frame, vec2 xy_f, vec2 frames, float d_scale, sampler2DArray depthTexture)
{
    ...
    float arr_pos = frame.y*frames.x + frame.x;
	vec3 uv_quad = vec3(uv_f, arr_pos);
	//paralax
	vec4 n_depth = texture(depthTexture, uv_quad);
	uv_f = xy_f * (0.5-n_depth.r) * d_scale + uv_f;
    uv_f = clamp(uv_f, vec2(0), vec2(1));
	//clamped parallaxed uvs
	return vec3(uv_f, arr_pos);
}

void fragment()
{
	vec3 uv_f1 = recalculateUV(uv_frame1, frame1, xy_frame1, imposterFrames, depth_scale, imposterDepthAlphaTexture);
	vec3 uv_f2 = recalculateUV(uv_frame2, frame2, xy_frame2, imposterFrames, depth_scale, imposterDepthAlphaTexture);
